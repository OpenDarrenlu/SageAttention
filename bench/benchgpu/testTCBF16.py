...
def test_tensorcore_bf16():
    # 检查环境
    if not torch.cuda.is_available():
        print("CUDA is not available. Please install PyTorch with CUDA support.")
        return
    
    # 检查是否支持 bf16 Tensor Core (Ampere+ 架构)
    major, minor = torch.cuda.get_device_capability()
    if major < 7:
        print(f"Your GPU (Compute Capability {major}.{minor}) has limited or no Tensor Core support.")
        print("bf16 Tensor Core requires Volta (7.0+) or newer architecture.")
        return
    
    print(f"Testing bf16 Tensor Core on GPU: {torch.cuda.get_device_name()}")
    ...
    Average Time: 2886.8323 ms | Runs: 20
    Measured Performance: 12.19 TFLOPS'''
    # 创建 bf16 张量
    ...
    for _ in range(num_runs):
        ...
        start.record()
        c = torch.matmul(a, b)  # 关键：bf16 矩阵乘法将自动使用 Tensor Core
        end.record()
    ...
    bf16_peak = fp32_peak * 2  # Ampere 架构: bf16 Tensor Core = FP32 * 2
    
    # 输出结果
    print(f"\n{'='*50}")
    print(f"bf16 Tensor Core Performance Test Results")
    print(f"{'='*50}")
    print(f"Matrix Size: {M}x{K} x {K}x{N}")
    print(f"Data Type: torch.bfloat16")
    ...
    print(f"- bf16 (Tensor Core): {bf16_peak:.2f} TFLOPS")
    print(f"\nUtilization:")
    print(f"- {tflops/bf16_peak*100:.1f}% of bf16 Tensor Core peak")
    print(f"{'='*50}")
    ...
    try:
        ...
        with profile(activities=[ProfilerActivity.CUDA],
            ...
            with record_function("bf16_matmul"):
                torch.matmul(a, b)
                torch.cuda.synchronize()
...
if __name__ == "__main__":
    print("="*50)
    print("GPU bf16 Tensor Core Performance Tester")
    print("="*50)
    
    # 确保禁用 TF32 (否则 bf16 测试可能使用 TF32)
    torch.backends.cuda.matmul.allow_tf32 = False
    torch.backends.cudnn.allow_tf32 = False
    print("Disabled TF32 to ensure pure bf16 measurement")
    
    # 设置环境变量确保使用 bf16
    os.environ["CUDA_TF32_OVERRIDE"] = "0"
    
    test_tensorcore_bf16()